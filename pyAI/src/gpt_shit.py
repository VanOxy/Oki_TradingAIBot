...
- Tier-0 TG features (no encoder yet): last TG snapshot + exponential time-decay
...
def build_observation(asset: AssetState, now_ts: float, kline_feats: Optional[list] = None,
    ...
    - TG part: last snapshot with exponential decay by staleness
    ...
    """
    tg = asset.tg
    w = decay_weight(now_ts, tg.ts)
    def d(x):
        v = _to_float(x)
        return 0.0 if v is None else float(v) * w
